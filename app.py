...
# Cache expensive resources to avoid re-instantiation on every rerun
@st.cache_resource
def get_stock_fetcher():
    """Get cached StockDataFetcher instance"""
    return StockDataFetcher()

@st.cache_resource
def get_ai_agents(model=None):
    """Get cached StockAnalysisAgents instance"""
    return StockAnalysisAgents(model=model or config.DEFAULT_MODEL_NAME)
...
@st.cache_data(ttl=300)  # 缓存5分钟
def get_quarterly_data_cached(symbol):
    """获取季报数据（带缓存）"""
    try:
...
@st.cache_data(ttl=300)  # 缓存5分钟
def get_fund_flow_data_cached(symbol):
    """获取资金流向数据（带缓存）"""
    try:
...
@st.cache_data(ttl=300)  # 缓存5分钟
def get_sentiment_data_cached(symbol, stock_data_hash):
    """获取市场情绪数据（带缓存）

    Args:
        symbol: 股票代码
        stock_data_hash: stock_data的hash值，用于缓存键
    """
    try:
        from market_sentiment_data import MarketSentimentDataFetcher
        sentiment_fetcher = MarketSentimentDataFetcher()
        # 需要重新获取stock_data，因为不能直接缓存DataFrame
        fetcher = get_stock_fetcher()
        stock_data = fetcher.get_stock_data(symbol, "1y")
        if isinstance(stock_data, dict) and "error" in stock_data:
            return {'data_success': False, 'error': stock_data.get('error')}
        return sentiment_fetcher.get_market_sentiment_data(symbol, stock_data)
    except Exception as e:
        return {'data_success': False, 'error': str(e)}

@st.cache_data(ttl=300)  # 缓存5分钟
def get_news_data_cached(symbol):
    """获取新闻数据（带缓存）"""
    try:
...
@st.cache_data(ttl=300)  # 缓存5分钟
def get_risk_data_cached(symbol):
    """获取风险数据（带缓存）"""
    try:
        fetcher = get_stock_fetcher()
...
def analyze_single_stock_for_batch(symbol, period, enabled_analysts_config=None, selected_model=None):
    ...
    try:
        ...
        # 2. 获取财务数据（使用缓存）
        fetcher = get_stock_fetcher()
        financial_data = get_financial_data_cached(symbol)
        ...
        if enable_fundamental and fetcher._is_chinese_stock(symbol):
            quarterly_data = get_quarterly_data_cached(symbol)
...
def run_stock_analysis(symbol, period):
    """运行股票分析（优化版：使用缓存和session_state避免重复获取）"""

    # 进度条
    progress_bar = st.progress(0)
    status_text = st.empty()

    # 检查是否需要重新获取数据（symbol或period变化时）
    need_refresh = (
        'current_symbol' not in st.session_state or
    ...
    try:
        ...
        is_chinese = fetcher._is_chinese_stock(symbol)

        # 2. 批量获取扩展数据（使用缓存函数，合并日志显示）
        status_text.text("📊 正在获取扩展数据（财务/季报/资金/情绪/新闻/风险）...")
        data_results = {}
        ...
        # 2.2 获取季报数据
        quarterly_data = None
        if enable_fundamental and is_chinese:
            quarterly_data = get_quarterly_data_cached(symbol)

        # 2.3 获取资金流向数据
        fund_flow_data = None
        if enable_fund_flow and is_chinese:
            fund_flow_data = get_fund_flow_data_cached(symbol)

        # 2.4 获取市场情绪数据
        sentiment_data = None
        if enable_sentiment and is_chinese:
            # 使用symbol作为缓存键的一部分
            sentiment_data = get_sentiment_data_cached(symbol, hash(str(period)))

        # 2.5 获取新闻数据
        news_data = None
        if enable_news and is_chinese:
            news_data = get_news_data_cached(symbol)

        # 2.6 获取风险数据
        risk_data = None
        if enable_risk and is_chinese:
            risk_data = get_risk_data_cached(symbol)
        ...
        if risk_data and risk_data.get('data_success'):
            ...
            if risk_data.get('shareholder_reduction') and risk_data['shareholder_reduction'].get('has_data'):
                risk_types.append("减持")
            if risk_data.get('important_events') and risk_data['important_events'].get('has_data'):
                risk_types.append("重要事件")
            if risk_types:
                data_summary.append(f"风险:{'+'.join(risk_types)}")

        if data_summary:
            st.info(f"✅ 数据获取完成：{', '.join(data_summary)}")
        elif not is_chinese:
            st.info("ℹ️ 美股仅支持基础数据和技术分析")
...
def display_history_records():
    ...
    # 初始化状态
    if 'detail_record_id' not in st.session_state:
        st.session_state.detail_record_id = None
    if 'monitor_record_id' not in st.session_state:
        st.session_state.monitor_record_id = None

    # 自定义 CSS：记录边框样式和按钮样式
    st.markdown("""
        <style>
        /* 记录容器边框 - 使用 Streamlit 的 st.columns 容器 */
    ...
    for record in filtered_records:
        ...
        with st.container():
            # 使用 HTML 创建边框容器
            st.markdown('<div class="record-row-container">', unsafe_allow_html=True)

            # 使用 columns 实现同一行布局：左侧信息，右侧按钮
            info_col, btn_col = st.columns([4, 1])

            with info_col:
                # 左侧：评级 + 股票信息 + 日期
                st.markdown(f"""
                    <span class="rating-text">{rating_color} {rating}</span>
                    <span class="stock-info">{record['stock_name']} ({record['symbol']}) - {record['analysis_date']}</span>
                """, unsafe_allow_html=True)
            ...
            with btn_col:
                ...
                with btn1:
                    if st.button("👀", key=f"view_{record_id}", help="查看详情"):
                        if st.session_state.detail_record_id == record_id:
                            st.session_state.detail_record_id = None
                        else:
                            st.session_state.detail_record_id = record_id
                            st.session_state.monitor_record_id = None
                        st.rerun()
                with btn2:
                    if st.button("➕", key=f"add_monitor_{record_id}", help="添加监测"):
                        if st.session_state.monitor_record_id == record_id:
                            st.session_state.monitor_record_id = None
                        else:
                            st.session_state.monitor_record_id = record_id
                            st.session_state.detail_record_id = None
                        st.rerun()
                with btn3:
                    if st.button("🗑️", key=f"delete_{record_id}", help="删除记录"):
        ...
        if st.session_state.detail_record_id == record_id:
            with st.container():
                st.markdown("---")
                display_record_detail_inline(record_id)
                st.markdown("---")
        ...
        if st.session_state.monitor_record_id == record_id:
            with st.container():
                st.markdown("---")
                record_data = db.get_record_by_id(record_id)
                if record_data:
                    display_add_to_monitor_dialog(record_data)
                else:
                    st.error("❌ 记录不存在")
                    st.session_state.monitor_record_id = None
                st.markdown("---")
            ...
            if submit:
                if new_entry_min > 0 and new_entry_max > 0 and new_entry_max > new_entry_min:
                    try:
                        ...
                        if 'monitor_record_id' in st.session_state:
                            st.session_state.monitor_record_id = None
                        if 'detail_record_id' in st.session_state:
                            st.session_state.detail_record_id = None
                        if 'expanded_record_id' in st.session_state:
...
def display_record_detail_inline(record_id):
    """在记录列表内联显示单条记录的详细信息（无返回按钮）"""
